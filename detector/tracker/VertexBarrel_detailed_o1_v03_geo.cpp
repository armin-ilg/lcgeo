      ...
      for (c_component.reset(); c_component; ++c_component) {
        ...
        if (isCurved) {
          ...
          Tube ele_box = Tube(rmin, rmin + thickness, component.length() / 2.,
                              -half_width + endOfStave.phi_offsets.back(), half_width + endOfStave.phi_offsets.back());
          ...
          for (auto& endOfStave_side : endOfStave_sides) { // Place it on both sides of the stave
            ...
            if (endOfStave.isCurved[i]) {
              ...
              z_pos = m.stave_length / 2. + endOfStave.lengths[i] / 2. + endOfStave.dzs[i] + endOfStave.z_offset +
                      ...
                      sin(endOfStave.tilts[i]) * endOfStave.thicknesses[i]; // Move slightly in z to avoid collision of
                                                                            // tilted end-of-stave volume with stave
